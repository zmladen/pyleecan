    ...
    if is_show_wires:
        ...
        for surface in surf_list:
            ...
            for jj in range(Zs):
                ...
                for wire in w_cord:
                    rotated_wire = wire * exp(1j * 2 * pi * jj / Zs)

                    circle = Circle(
                        (rotated_wire.real, rotated_wire.imag),
                        self.get_Wwire() / 2,
                    )
                    circles.append(circle)

        # Use PatchCollection to improve performance
        wire_collection = PatchCollection(
            circles, facecolor="none", edgecolor="black", zorder=2
        )

        ax.add_collection(wire_collection)
