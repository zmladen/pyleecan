...
def get_Kwoh(self):
    """Return the wire width (Kwoh) from the conductor if it exists."""
    if self.parent is not None:
        if hasattr(self.parent, "winding") and self.parent.winding is not None:
            if (
                ...
                if hasattr(conductor, "Kwoh"):
                    return conductor.Kwoh
                else:
                    raise AttributeError(
                        f"Error: 'Conductor' is a base class and has no attribute 'Kwoh'. Assign different conductor type to winding."
                    )
            else:
