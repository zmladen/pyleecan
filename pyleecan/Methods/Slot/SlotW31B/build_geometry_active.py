...
def build_geometry_active(self, Nrad, Ntan, alpha=0, delta=0):
    ...
    self : SlotW31B
        A SlotW31B object
    Nrad : int.
        Number of radial layer. This is defined in the winding modul
    Ntan : int
        Number of tangentiel layer. This is defined in the winding modul
    alpha : float
    ...
    tan_list = list()
    if Ntan == 2:
        top_surf, middle_surf = surf_act.split_line(
            self.i_tan / 2 * 1j,
            self.i_tan / 2 * 1j + 100,
            is_join=True,
            prop_dict_join=None,
        ...
        middle_surf, bot_surf = middle_surf.split_line(
            -self.i_tan / 2 * 1j,
            -self.i_tan / 2 * 1j + 100,
            is_join=True,
            prop_dict_join=None,
    ...
    # Rad split (Radial gap missing. add it later if necessary!)
    surf_list = list()
